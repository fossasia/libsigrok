...
SR_PRIV int pslab_probe_channels(struct sr_dev_inst *sdi)
{
	...
	while (TRUE) {
		...
		if (!pslab_check_packet_prefix(packet, packet_len)) {
...
SR_PRIV int pslab_request_packet(const struct sr_dev_inst *sdi)
{
	...
	libusb_fill_bulk_transfer(devc->out_transfer, usb->devhdl, EP_OUT,
			...
			pslab_receive_transfer, (void *)sdi, 100);
...
SR_PRIV gboolean pslab_check_packet_prefix(unsigned char *buf, int len)
{
...
SR_PRIV void pslab_receive_packet(const struct sr_dev_inst *sdi)
{
	struct dev_context *devc;
