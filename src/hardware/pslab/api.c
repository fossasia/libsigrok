...
static GSList *scan(struct sr_dev_driver *di, GSList *options)
{
    ...
    /*
     * usb 2-2: new full-speed USB device number 48 using xhci_hcd
     * usb 2-2: New USB device found, idVendor=10c4, idProduct=ea60, bcdDevice= 1.00
     * usb 2-2: New USB device strings: Mfr=1, Product=2, SerialNumber=3
     * usb 2-2: Product: CP2102N USB to UART Bridge Controller
     ...
     */
		...
		/* Hardcode the product name for now. */
		if (strcmp(product, "CP2102N USB to UART Bridge Controller"))
			continue;

		sdi = g_malloc0(sizeof(struct sr_dev_inst));
		sdi->status = SR_ST_INACTIVE;
		sdi->vendor = g_strdup("FOSSASIA");
		sdi->model = g_strdup("PSLab v6");
...
static void receive_data(struct sr_dev_inst *sdi, unsigned char *data, int len)
{
	...
	if (!pslab_check_packet_prefix(devc->reply, devc->reply_size))
		return;
